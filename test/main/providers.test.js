...
test('deepseek getCodeBlockLanguage 从 data-language 提取', () => {
  const pre = {
    getAttribute: (n) => n === 'data-language' ? 'lingya' : '',
    closest: () => null,
    querySelector: () => null,
    classList: [],
  };
  assert.strictEqual(deepseek.getCodeBlockLanguage(pre), 'lingya');
});

test('claude getCodeBlockLanguage 从 language-* 提取', () => {
  const pre = {
    querySelector: () => ({ className: 'language-lingya' }),
  };
  assert.strictEqual(claude.getCodeBlockLanguage(pre), 'lingya');
});
...
test('chatgpt getCodeBlockLanguage 从 class 提取', () => {
  ...
  assert.strictEqual(chatgpt.getCodeBlockLanguage(pre), 'lingya');
});
...
test('渲染进程经注入的 userData 路径可加载自定义 Provider', () => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'lingya-cp-'));
  ...
  process.argv = savedArgv.concat(['--lingya-user-data=' + tmp]);
