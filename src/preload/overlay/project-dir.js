...
function initProjectDirSection() {
  ...
  setTimeout(() => {
    const changeBtn = document.getElementById('lingya-btn-change-dir');
    if (changeBtn) {
...
function updateProjectDirDisplay(dirPath) {
  ...
  // 记住最近一次有效目录，供"新会话自动注入"复用（只写有效值，不被 null 覆盖）
  if (dirPath && String(dirPath).trim()) {
    try { localStorage.setItem('lingya-last-project-dir', dirPath); } catch (_) {}
  }
  const display = document.getElementById('lingya-project-dir-display');
  const section = document.querySelector('.lingya-project-dir-section');
  if (display) {
    const span = display.querySelector('.lingya-dir-path');
    if (span) {
