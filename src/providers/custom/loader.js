...
if (isRenderer) {
  ...
  const arg = argv.find(a => a.startsWith('--lingya-user-data='));
  if (arg) rendererUserDataPath = arg.slice('--lingya-user-data='.length);
}
...
function getCacheHolder() {
  ...
  if (!process.__lingyaCustomProvidersCache ||
      process.__lingyaCustomProvidersCache.key !== key) {
    process.__lingyaCustomProvidersCache = { key, providers: null };
  }
  return process.__lingyaCustomProvidersCache;
}
